...
import BitcoinResolverABI from "@/utils/BitcoinResolverABI.json";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const secretHash = `0x${createHash("sha256")
    ...
    try {
      ...
      try {
        if (swap_type === "native") {
          bitcoinResponse = await triggerNativeBitcoinHandler(
            htlcHash,
            amount,
            to_token,
          );
...
async function triggerNativeBitcoinHandler(
  htlcHash: string,
  amount: string,
